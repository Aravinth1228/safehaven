...
contract TouristSafety {
     ...
     * @return touristId The generated unique tourist ID
     */
     ...
     * @dev Creates an ID in format: TID-XXXXXX-XXXX
     */
    function _generateTouristId() private view returns (string memory) {
        // Generate a pseudo-random 10-character hex string
        ...
        // Convert first 10 hex characters to string
        bytes memory result = new bytes(15); // TID-XXXXXX-XXXX
        result[0] = 'T';
        result[1] = 'I';
        result[2] = 'D';
        result[3] = '-';
        ...
        for (uint256 i = 0; i < 6; i++) {
            result[4 + i] = chars[uint8(hash[i]) % 16];
        }
        ...
        for (uint256 i = 0; i < 4; i++) {
            result[11 + i] = chars[uint8(hash[6 + i]) % 16];
        }
    ...
    function _checkDangerZoneEntry(
        ...
        for (uint256 i = 0; i < dangerZones.length; i++) {
            ...
            // Distance squared (avoiding sqrt for gas efficiency)
            int256 distanceSquared = (latDiff * latDiff) + (lngDiff * lngDiff);
            
            // Radius squared in coordinate units (1 meter ≈ 10 units at 1e6 precision)
            int256 radiusSquared = int256(zone.radius * zone.radius * 100);
            ...
            if (distanceSquared <= radiusSquared) {
                ...
                break; // Only trigger once per location update
            }
        }
    }
     ...
     * @return zoneId The generated zone ID
     */
    function createDangerZone(
        ...
        zoneCounter++;
        string memory zoneId = string(abi.encodePacked(
            "ZONE-",
            _uint2str(zoneCounter)
        ));
    ...
    /**
     * @notice Create an emergency alert (internal)
     */
    function _createEmergencyAlert(
        ...
        alertCounter++;
        string memory alertId = string(abi.encodePacked(
            "ALERT-",
            _uint2str(alertCounter)
        ));
